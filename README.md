...
**EURGBP 1w OHLCV forex historical data** — ultra high-quality 1w OHLCV for **Euro / British Pound**. Clean `time, open, high, low, close, volume` CSV for backtesting, algorithmic trading and quantitative research.
...
> **Sample on GitHub** · `EURGBP_1w.csv` (7 rows, `2026-07-16` -> `2026-08-27`, 0.55 KB). **Full archive on [getdata.finance](https://getdata.finance/datasets/eurgbp)** — **2,696** `1w` rows (full `1m`: 5,322,882), **11 timeframes**, `1975-01-02` -> `2026-08-27`.
...
| 1w rows | 7 | **2,696** |
| Size | 0.55 KB | full ZIP on [getdata.finance](https://getdata.finance/datasets/eurgbp) |
...
df = pd.read_csv('EURGBP_1w.csv', parse_dates=['time'])
